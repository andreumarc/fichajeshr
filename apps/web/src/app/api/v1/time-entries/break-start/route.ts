...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (lastEntry.type === TimeEntryType.BREAK_START) {
      return NextResponse.json({ message: 'Ya tienes una pausa activa.' }, { status: 400 });
    }

    const entry = await createTimeEntry(employeeId, user!.companyId!, dto, TimeEntryType.BREAK_START, meta);

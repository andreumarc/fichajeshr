...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!lastEntry || lastEntry.type !== TimeEntryType.BREAK_START) {
      return NextResponse.json({ message: 'No hay pausa activa para finalizar.' }, { status: 400 });
    }

    const entry = await createTimeEntry(employeeId, user!.companyId!, dto, TimeEntryType.BREAK_END, meta);
